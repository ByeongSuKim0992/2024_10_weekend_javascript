const position = ["positiony", "z-index", "top", "button", "right", "Flexbox properties", "float", "clear"];
const display = ["display", "opacity", "transform"];
const clipping = ["overflow", "clipping"];
const animation = ["animation", "transition"];
const box = ["margin", "box-shadow", "boder", "boder-radius", "box-sizing", "width", "height", "padding"];
...
const makeTemplate = (titleName, color, buttonList) => {
    ...
    buttonList.forEach((v) => {
        ...
        btn.style.cssText = `background-color: ${color};border:1px solid ${color}; border-radius:5px; padding:10px 10px 7px;weight:blod;`;
        btn.innerText = v;
...
makeTemplate("Box Model (from ourside in", "#98eccc", box);
